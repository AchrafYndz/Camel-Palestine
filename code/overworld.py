...
class Overworld:
    ...
    def input(self):
        ...
        if not self.moving:
            if (keys[pygame.K_d] or keys[pygame.K_RIGHT]) and self.current_level < self.max_level and self.allow_input:
                self.move_direction = self.get_movement_data("forward")
                self.current_level += 1
                self.moving = True
            elif (keys[pygame.K_a] or keys[pygame.K_LEFT]) and self.current_level > 0 and self.allow_input:
